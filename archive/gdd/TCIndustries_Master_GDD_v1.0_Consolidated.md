...
**Version:** 1.0 — Consolidated Master GDD  
**Consolidation Date:** 2026-08-25  
**Author:** OpenAI GPT-5.6 Luna  
**Document Purpose:** Establish the single primary, internally coherent design reference for TCIndustries by synthesising multiple first-pass Master GDD sources.  
**Authority Statement:** This document is the current primary design authority for TCIndustries. It supersedes the individual first-pass Master GDDs as the consolidated reference. Status tags are authoritative. No status is silently promoted from any source.  

**Source Documents Inspected:**
1. `TCIndustries Master Game Design Document v0.1 - gpt-5.6-terra-medium.md`
2. `TCIndustries_Master_GDD-chatgptv1-1.md` (identical to source 4)
3. `TCIndustries_Master_GDD_v1.0-claude-opus-4-6-thiking.md`
4. `TCIndustries_Master_GDD-claude1.md`
5. Supporting reference: `Tiwakings Craftworld Industries — Master Game Design Document v0.1-chatgpt1.md`
...
| Change Control | Status changes require explicit project/human approval. |
...
**Critical Rule:** Never silently promote PROPOSED, TBD, PROTOTYPE, DEFERRED, or ASSUMPTION material into LOCKED. Status changes require explicit documented human/project approval.
...
1. Explicit human/project rulings have highest authority.
2. LOCKED rules from sources that explicitly attribute them to project brief or human direction are treated as LOCKED.
3. Consistent design decisions independently proposed across multiple documents remain PROPOSED unless higher authority exists.
4. Prototype behaviour is evidence, not design authority. GDD rules govern over prototype behaviour unless a later explicit human ruling changes that relationship.
5. Historical SWG behaviour is inspiration only. It is never automatically a TCIndustries rule.
...
**Status: LOCKED** (derived from interdependence and specialisation principles)
...
**Status: LOCKED** (anti-goal reinforcement)
...
**Status: LOCKED** (derived from LOOP-002 and anti-goals)
...
**Status: LOCKED** (derived from non-combat viability)
...
| MFG-004 | Anti-Factorio drift | LOCKED | Anti-goal | Manufacturing | Core |
...
1. Confirm or revise any LOCKED items that require human re-validation.
2. Resolve specialisation budget / skill-cap model (OQ-001) — foundational for interdependence.
3. Define resource attribute model and lifecycle shape (OQ-002, OQ-003) without final numbers.
4. Define crafting schematic + experimentation shape (OQ-004).
5. Define manufacturing constraints that enforce MFG-001/MFG-004.
6. Specify ownership, provenance, and vendor basics.
7. City and organisation minimal viable rules.
8. Combat scope decision (what is in / out for first persistent version).
9. Transportation and logistics minimal rules.
10. Numeric tuning and simulation passes only after structural shapes are stable.
11. Prototype reconciliation pass when prototype is available for inspection.
...
| Status Tag | Explicit marker (LOCKED, PROPOSED, TBD, etc.) controlling the authority of a design statement. |

---

# 36. Consolidation Notes / Provenance Appendix

## Major Synthesis Decisions

1. **LOCKED core vision and pillars** were retained from consistent explicit language across all four primary sources and the supporting document. These are treated as project-level decisions, not LLM inventions.
2. **Detailed taxonomies, skill lists, attribute examples, and mechanical shapes** that appeared in multiple sources were retained as **PROPOSED**. Agreement among LLM documents does not promote them to LOCKED.
3. **Numeric values** were universally avoided. No source provided authoritative numbers; all such values remain TBD.
4. **Prototype references** were preserved as PROTOTYPE/ASSUMPTION. No prototype was inspected during this consolidation.
5. **Historical SWG material** was strictly separated and marked HISTORICAL. No Star Wars-specific content was carried into TCIndustries rules.
6. **Identical sources** (`TCIndustries_Master_GDD-chatgptv1-1.md` and `TCIndustries_Master_GDD-claude1.md`) were treated as a single document.
7. **Structure** was normalised toward a comprehensive professional Master GDD layout drawing from the most complete source (claude-opus v1.0) while incorporating consistent material from the others.
8. **Rule IDs** were normalised where sources used compatible identifiers (VIS-, PIL-, RES-, CRFT-, etc.). No material collisions of ID meaning required renumbering beyond normal synthesis.

## Unresolved Conflicts

- No major LOCKED-vs-LOCKED conflicts were found among the sources. Differences were primarily in level of detail, section organisation, and the confidence with which PROPOSED material was written.
- Some sources mark certain high-level economic or progression principles as LOCKED while others treat closely related statements as PROPOSED. In such cases the more conservative (PROPOSED) status was preferred unless the language clearly indicated project-brief origin.
- Exact scope of “first playable” systems remains open (OQ-015).

## Material Intentionally Excluded

- Duplicate section text and repeated explanatory prose from the four sources.
- Overly specific example numbers or formula sketches that appeared in some sources without authority.
- Any residual Star Wars-specific terminology that had not already been filtered.
- Project-management or implementation-schedule content that does not belong in a design document.
- Speculative future systems marked DEFERRED in sources that did not add unique design value.
...
10. Confirmation that the current set of LOCKED vision/pillar statements remains accurate after human review.
11. Reconciliation of any existing prototype against this GDD.

---

**End of Consolidated Master GDD v1.0**

This document is intended to serve as the single primary design reference. Future system-specific specifications should build upon, and remain consistent with, the LOCKED principles and status discipline established here.
