...
- Master boxes cost a flat 10 points (capstone of the profession, unlocked after all 4 trees are complete)
- Dropping skills refunds skill points (instant respec allowed)

**[ASSUMPTION] Per-tier skill point cost pattern:** the four boxes within a tree follow an increasing cost pattern that scales by profession category, calibrated so the "~40 / ~80 / ~120 points to Master" totals stated throughout Section 8 are internally consistent rather than approximate in name only:

| Profession category | Tier I / II / III / IV cost (per tree) | × 4 trees | + Master (10) | Total |
|---|---|---|---|---|
| Basic (Section 8.2) | 1, 2, 2, 3 | 32 | 42 | ≈ 40 |
| Elite, single-prerequisite (most of Section 8.3) | 3, 4, 5, 6 | 72 | 82 | ≈ 80 |
| Elite, multi-prerequisite (Bounty Hunter, Commando, Smuggler, TKA) | 5, 7, 7, 8 | 108 | 118 | ≈ 120 |

**Example Skill Box Costs (corrected):**

```text
Marksman Tree (Basic — 1/2/2/3 pattern):
├─ Novice Marksman (0 points) [FREE ENTRY]
├─ Ranged Accuracy I (1 point)
├─ Ranged Accuracy II (2 points)
├─ Ranged Accuracy III (2 points)
├─ Ranged Accuracy IV (3 points)
[...same 1/2/2/3 pattern across the remaining 3 trees, totaling 32 points,
 + Master Marksman (10 points) = 42 points]

Pistoleer Tree (Elite, single-prerequisite — 3/4/5/6 pattern):
├─ Pistol Accuracy I (3 points)
├─ Pistol Accuracy II (4 points)
├─ Pistol Accuracy III (5 points)
├─ Pistol Accuracy IV (6 points)
[...same 3/4/5/6 pattern across the remaining 3 trees, totaling 72 points,
 + Master Pistoleer (10 points) = 82 points]
```

*Editorial note: an earlier draft of this document illustrated a single 2/3/4/5 pattern for all professions regardless of category, which does not reconcile against the "~40 / ~80 / ~120" totals stated per-profession in Section 8. The tiered pattern above is the corrected, authoritative version — use it, not the flat pattern, if the two ever appear to conflict elsewhere.*
...
**[ASSUMPTION] XP and Credit Cost Formula:** rather than hand-authoring several hundred individual XP/credit values (one per skill box across every profession), both are derived deterministically from the box's skill-point cost (Section 6.3.1), so any implementation can generate a complete, internally consistent table programmatically:

```text
XP_cost = 1000 × (skill_point_cost)²
Credit_cost = round(XP_cost / 20, nearest 10)
```

Example outputs: a 1-point box costs 1,000 XP / 50 credits; a 6-point box costs 36,000 XP / 1,800 credits; any 10-point Master box costs 100,000 XP / 5,000 credits — consistent with the "~1k for early boxes, ~100k for Master boxes" guidance above and the training-cost sink range in Section 12.3. Novice boxes are always 0 XP / 0 credits.
...
**[ASSUMPTION] Within-band result and point-allocation mechanics:** the band probabilities above are the baseline (zero points allocated, base tool). In implementation:

- The magnitude within a band is a uniform random draw across that band's stated range (e.g., a Critical Success draws uniformly between +5% and +10%; nothing weights it toward either end).
- Critical Success (10%), Failure (8%), and Critical Failure (2%) probabilities are fixed — they do not move with point allocation or tool tier.
- The remaining 80% is split between Normal (starts at 30%) and Success (starts at 50%). Each experimentation point allocated to that attribute shifts 3 percentage points from Normal to Success. Each tool-tier bonus (Section 10.7.2: +10% crafted tool, +20% station) shifts that same percentage, in points, from Normal to Success, stacking additively with point allocation.
- Normal is floored at 5% (so Success caps at 75% even at maximum investment) — Normal never reaches zero regardless of how much is invested, preserving some chance of a mediocre result at every skill level.
...
**[ASSUMPTION] General weighting rule for schematics without a hand-tuned matrix:** a complete, per-schematic resource-stat weighting matrix (which named resource substat affects which item stat, for every schematic in the game) is a large data-authoring task, not a design-philosophy question, and is left for content authoring rather than fully enumerated here. Until a schematic has a hand-tuned matrix, apply this general rule: every schematic designates one primary resource slot (the ingredient contributing the most units) and up to two secondary slots. Each contributes to the item's base stat via its Overall Quality (OQ):

```text
Final_Base_Stat = Schematic_Base_Stat
                 + (Primary_Resource_OQ × 0.5)
                 + Σ(Secondary_Resource_OQ × 0.2)
```

Crafted component resources (e.g., a Weapon Barrel) contribute through their own already-computed stats rather than a separate multiplier, so quality compounds through the crafting chains in Section 8.4. Mapping specific named substats (Malleability, Conductivity, etc.) to specific item stats per schematic, rather than using OQ as a blanket proxy, is flagged **[EXPANSION]** — a valuable balance-fidelity refinement, not something that blocks MVP.
