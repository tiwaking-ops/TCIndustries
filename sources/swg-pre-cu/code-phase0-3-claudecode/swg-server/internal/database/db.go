...
func (db *DB) migrate() error {
	migrations := []string{
		...
		// 008: lairs table (GDD 17.3, 9.5.3, Phase 3)
		...
		// 009: creature_instances table (GDD 17.3, 28.3, Phase 3)
		`CREATE TABLE IF NOT EXISTS creature_instances (
			id TEXT PRIMARY KEY,
			template_id TEXT NOT NULL,
			lair_id TEXT REFERENCES lairs(id) ON DELETE SET NULL,
			planet TEXT NOT NULL,
			pos_x REAL NOT NULL,
			pos_y REAL NOT NULL,
			pos_z REAL NOT NULL,
	...
	// 010: clone-binding columns on characters (GDD 9.6.3, Phase 3).
	// Uses an idempotent helper rather than a plain ALTER TABLE, since
	// SQLite's ALTER TABLE ADD COLUMN errors on re-run and migrate() runs
	// on every server startup, not just the first.
	cloneCols := map[string]string{
		"clone_bind_planet": "TEXT DEFAULT 'tatooine'",
		"clone_bind_x":      "REAL DEFAULT 3528.0",
		"clone_bind_y":      "REAL DEFAULT 5.0",
		"clone_bind_z":      "REAL DEFAULT -4804.0",
	}
	for col, def := range cloneCols {
		if err := db.addColumnIfNotExists("characters", col, def); err != nil {
...
// addColumnIfNotExists adds a column to a table only if it doesn't already
// exist, checked via PRAGMA table_info. Needed because SQLite's ALTER TABLE
// ADD COLUMN has no IF NOT EXISTS clause, and migrate() runs on every
// startup, so a plain ALTER TABLE would error on the second run.
