...
type Client struct {
	...
	// Phase 3: In-memory authoritative combat state (GDD 9.2.1). Loaded at
	// world_enter, mutated on damage/regen, persisted on every meaningful
	// change (combat events are lower-frequency than position ticks, so
	// unlike position there's no need for a periodic-only persistence
	// window here -- every change is written through).
	Combat combat.HAMPoolState
}
...
type WorldHandler struct {
	...
	// Phase 3: creature instances, keyed by instance ID. Loaded from DB at
	// startup, kept authoritative in-memory, persisted on damage/death
	// (same pattern as player position: in-memory authoritative + periodic
	// persistence, not a DB round-trip per tick).
	creatures map[string]*creatures.Instance
}
...
func NewWorldHandler(db *database.DB) *WorldHandler {
	return &WorldHandler{
		...
		creatures: make(map[string]*creatures.Instance),
	}
}
...
func (h *WorldHandler) readPump(client *Client) {
	...
	for {
		...
		case protocol.MsgCombatAction:
			h.handleCombatAction(client, message)
		case protocol.MsgSetPosture:
			h.handleSetPosture(client, message)
		case protocol.MsgSetStance:
			h.handleSetStance(client, message)
		case protocol.MsgClone:
			h.handleClone(client)
		case protocol.MsgRevive:
			h.handleRevive(client, message)
		default:
...
func (h *WorldHandler) handleEnterWorld(client *Client, raw []byte) {
	...
	// Compute species max HAM (for the max-pool values only)
	speciesHAM := species.ComputeHAM(species.SpeciesID(charWithHAM.Species))

	// Load actual persisted combat state (current HAM, wounds, BF, posture,
	// stance) rather than always sending fresh max values. Before Phase 3
	// this always sent species.ComputeHAM() directly, which was harmless
	// while nothing could ever change HAM -- now that combat exists, a
	// character logging back in after taking damage needs to see their
	// real current state, not a full-health reset.
	combatState, err := h.db.GetCombatState(charWithHAM.ID)
	if err != nil {
		h.sendError(client, "failed to load combat state")
		return
	}
	client.Combat = *combatState
	...
	worldEnter := protocol.WorldEnterMsg{
		...
		HAM: protocol.HAMState{
			Health:       speciesHAM.Health,
			Strength:     speciesHAM.Strength,
			Constitution: speciesHAM.Constitution,
			Action:       speciesHAM.Action,
			Quickness:    speciesHAM.Quickness,
			Stamina:      speciesHAM.Stamina,
			Mind:         speciesHAM.Mind,
			Focus:        speciesHAM.Focus,
			Willpower:    speciesHAM.Willpower,
		},
	}
	h.send(client, protocol.MsgWorldEnter, worldEnter)

	// Phase 3: also push current combat state (current/max/wounds/BF/
	// posture/stance) via ham_update, since WorldEnterMsg's HAMState type
	// predates current-vs-max and stays unchanged for backward compat.
