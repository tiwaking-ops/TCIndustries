...
func main() {
	...
	fmt.Println("\n--- Test 2: Movement broadcast to nearby player ---")
	// The server validates implied speed as distance / dt since the
	// player's last known position (set at world_enter). Sending a move
	// immediately after entry gives a dt of ~50ms, so even a genuine 1m
	// move implies ~20 m/s and is correctly rejected by the server's
	// anti-cheat check (max 10.5 m/s, see internal/world/movement.go).
	// Wait first so the implied speed of a slow 1m move is plausible.
	time.Sleep(200 * time.Millisecond)
